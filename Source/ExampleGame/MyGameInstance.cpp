		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			if (Authorization)
			{
				...
				for (int32 b = 0; b <= teamCount; b++) {
					...
					int32 currentTeamPlayerIndex = 0;
					...
					for (int32 i = 0; i < MatchInfo.players.Num(); i++) {
						if (b == MatchInfo.players[i].teamId) {
							...
							// I think it is safe to set this here too.
							MatchInfo.players[i].teamPlayerIndex = currentTeamPlayerIndex;
				...
				// get the custom metaMatch info if available.  Hang on to it for later.
				// metaMatch info is generated by the metagame server, and passed through when the server is created.
				// It allows you to send any information from the metagame to the actual match.
				// Like:  this zone contains turrets, a zone terrain type, or any other information you want. 

				JsonParsed->TryGetStringField("metaMatchCustom", metaMatchCustom);

				//FString metaMatchCustomFake = "{\"control\": 23, \"energy\": 61,\"mines\" : \"TRUE\",\"refinery\" : \"TRUE\",\"regionKeyId\" : 5634472569470976,\"verticalIndex\" : 1,\"horizontalIndex\" : 2,\"validZone\" : \"TRUE\",\"customtitle\" : \"TEST HQ\",\"title\" : \"Energy Zone\",\"mapTitle\" : \"US WEST LA MAP 1\",\"fortifications1\" : \"TRUE\",\"factory\" : \"TRUE\",\"mapKeyId\" : 5631986051842048,\"factionTag\" : \"[TEST]\",\"shields\" : \"TRUE\",\"userCaptainKeyId\" : 5629499534213120,\"controlled\" : \"TRUE\",\"teamTitle\" : \"NULL\",\"powerplant\" : \"TRUE\",\"materials\" : 29,\"key_id\" : 5707090131681280,\"turrets\" : \"TRUE\"}";
				// Auto parse it into our zoneDetail struct
				FJsonObjectConverter::JsonObjectStringToUStruct<FZoneDetails>(
				...
				if (PlayerAuthorized) {
					...
					if (ActivePlayer)
					{
						...
						if (playerS)
						{
							...
							ActivePlayer->playerID = thisPlayerState->GetPlayerId();
						...
						// Set the timer
						// Increasing this to 10 seconds because sometimes playerState is not ready until 7 seconds.  
						GetWorld()->GetTimerManager().SetTimer(ActivePlayer->GetPlayerInfoDelayHandle, ActivePlayer->GetPlayerInfoTimerDel, 10.f, false);
	...
	else
	{
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			if (Authorization)
			{
				...
				// TODO:  SafeToContinue struff from FORMUP - Skipping for now until I can compile successfully since it is a lot of logic.
	...
	if (IsRunningDedicatedServer()) {
		...
		if (UEtopiaMode == "competitive") {
			...
			if (CurrentActivePlayer) {
				...
				if (playerS)
				{
					UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [UMyGameInstance] [DeActivatePlayer] - Found playerState"));
				...
				else
				{
					if (checkOnly == "true") {
						...
						if (playerS)
						{
							...
							UE_LOG(LogTemp, Log, TEXT("playerS->PlayerId: %d"), thisPlayerState->GetPlayerId());
...
bool UMyGameInstance::VendorItemCreate(AMyPlayerController* playerController, FString VendorKeyId, int32 index, int32 pricePerUnit)
{
	...
	if (myPlayerState)
	{
		...
		// Send DTID too!
		PlayerJsonObj->SetNumberField("dataTableId", myPlayerState->InventorySlots[index].DataTableId);
...
bool UMyGameInstance::AttemptRemoveVendor(FString VendorKeyId)
{
	UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [UMyGameInstance] AttemptRemoveVendor"));

	// A client has attempted to interact with a vendor, but the backend could not find it.
	// Most likely the timeout expired, and it was deleted from the backend.
	// It is still loading into the game server becuase of RAMA save.
	// So we need to just douple check that this vendor does exist in game, and check the backend to verify that it is not there.
	// then, just delete it
	// maybe send a text chat to the player that tried to interact with it?


	TArray<AActor*> FoundActors;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AMyBaseVendor::StaticClass(), FoundActors);

	for (int32 i = 0; i < FoundActors.Num(); i++)
	{
		AMyBaseVendor* thisVendorActor = Cast<AMyBaseVendor>(FoundActors[i]);

		if (thisVendorActor)
		{
			UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [UMyGameInstance] VendorAttemptRemovalNotFound - found a vendor"));

			if (thisVendorActor->VendorKeyId == VendorKeyId)
			{
				UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [UMyGameInstance] VendorAttemptRemovalNotFound - found the indicated vendor"));

				// Fire off the http request to check it
				...
				ServerJsonObj->SetStringField("key_id", VendorKeyId);

				FString JsonOutputString;
				TSharedRef< TJsonWriter<> > Writer = TJsonWriterFactory<>::Create(&JsonOutputString);
				FJsonSerializer::Serialize(ServerJsonObj.ToSharedRef(), Writer);

				// _ah/api/vendors/v1/get

				FString APIURI = "/api/v1/server/vendor/" + VendorKeyId;

				bool requestSuccess = PerformJsonHttpRequest(&UMyGameInstance::VendorAttemptRemovalComplete, APIURI, JsonOutputString, "");
...
void UMyGameInstance::VendorAttemptRemovalComplete(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	...
	{
		UE_LOG(LogTemp, Log, TEXT("Completed VendorAttemptRemovalNotFoundComplete [%s] Url=[%s] Response=[%d] [%s]"),
			*HttpRequest->GetVerb(),
			*HttpRequest->GetURL(),
			HttpResponse->GetResponseCode(),
			*HttpResponse->GetContentAsString());
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			if (Authorization)
			{
				...
				bool Success = JsonParsed->GetBoolField("success");
				if (!Success) {
					UE_LOG(LogTemp, Log, TEXT("Success FALSE"));
					// this is what we expect to see if the client was telling the truth.

					//get the vendorKeyId out of the json
					FString vendorKeyId = JsonParsed->GetStringField("key_id");
					...
					for (int32 i = 0; i < FoundActors.Num(); i++)
					{
						...
						if (thisVendorActor)
						{
							...
							if (thisVendorActor->VendorKeyId == vendorKeyId)
							{
								...
								thisVendorActor->Destroy();
							}
						}
					}

					// Send a text to the player, somehow?
